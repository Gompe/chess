...
pub type ChessBoard=InplaceChessBoard;

#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct InplaceChessBoard {
    board : [Option<ColorPiece>; 64],
    turn_color : Color
}

#[allow(dead_code)]
impl InplaceChessBoard {
    ...
    pub fn get_game_status_from_precomputed(&self, allowed_moves: &Vec<Move>) -> ChessStatus {
    ...
    fn inplace_free_squares_in_direction(&self, coordinate: &Square, delta_row: i8, delta_col: i8, output: &mut Vec<Square>) {
    ...
    pub fn inplace_squares_attacked_by_rook(&self, coordinate: &Square, output: &mut Vec<Square>) {
    ...
    pub fn inplace_squares_attacked_by_bishop(&self, coordinate: &Square, output: &mut Vec<Square>) {
    ...
    pub fn inplace_squares_attacked_by_queen(&self, coordinate: &Square, output: &mut Vec<Square>) {
        self.inplace_squares_attacked_by_bishop(coordinate, output);
        self.inplace_squares_attacked_by_rook(coordinate, output)
    }

    pub fn inplace_squares_attacked_by_knight(&self, coordinate: &Square, output: &mut Vec<Square>) {
    ...
    pub fn inplace_squares_attacked_by_king(&self, coordinate: &Square, output: &mut Vec<Square>) {
        output.append(&mut vec![
            coordinate.add( 0, -1),
            coordinate.add( 0,  1),
            coordinate.add( -1, -1),
    ...
    pub fn inplace_squares_attacked_by_piece(&self, coordinate: &Square, output: &mut Vec<Square>) {
        ...
        match color_piece.get_piece() {
            ...
            Piece::Queen => {
                self.inplace_squares_attacked_by_queen(coordinate, output)
            },
            Piece::Knight => {
                self.inplace_squares_attacked_by_knight(coordinate, output)
            },
            Piece::King => {
                self.inplace_squares_attacked_by_king(coordinate, output)
            },
            Piece::Pawn => {
    ...
    fn inplace_get_potential_moves(&self, coordinate: &Square, output: &mut Vec<Move>) {
        let color_piece = match self.get_square_content(coordinate) {
            Some(color_piece) => color_piece,
            None => return 
        };
        ...
        if color_piece.get_piece() == Piece::Pawn {
            ...
            output.append(&mut moves.iter().filter_map(|&coordinate| {
    ...
    pub fn is_square_attacked_by_color(&self, coordinate: &Square, color: Color) -> bool {
        ...
        let check_piece = |output: &Vec<Square>, piece| {
            output.iter().map(|square| self.get_square_content(square)).any(|x| x == Some(ColorPiece::new(color, piece)))        
        };
    ...
    pub fn get_allowed_moves(&self, color: Color) -> Vec<Move> {
        ...
        let mut maybe_moves = Vec::with_capacity(64);
