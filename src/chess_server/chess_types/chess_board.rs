...
impl SmallVecChessBoard {
    ...
    pub fn inplace_squares_attacked_by_rook(&self, coordinate: &Square, output: &mut SquareContainer) {
    ...
    pub fn inplace_squares_attacked_by_bishop(&self, coordinate: &Square, output: &mut SquareContainer) {
    ...
    pub fn inplace_squares_attacked_by_queen(&self, coordinate: &Square, output: &mut SquareContainer) {
        self.inplace_squares_attacked_by_bishop(coordinate, output);
        self.inplace_squares_attacked_by_rook(coordinate, output)
    }

    pub fn inplace_squares_attacked_by_knight(&self, coordinate: &Square, output: &mut SquareContainer) {
    ...
    pub fn inplace_squares_attacked_by_king(&self, coordinate: &Square, output: &mut SquareContainer) {
        let temp: SmallVec<[_; 16]> = smallvec![
            coordinate.add( 0, -1),
            coordinate.add( 0,  1),
            coordinate.add( -1, -1),
    ...
    pub fn inplace_squares_attacked_by_piece(&self, coordinate: &Square, output: &mut SquareContainer) {
        ...
        match color_piece.get_piece() {
            ...
            Piece::Pawn => {
                match color_piece.get_color() {
                    Color::White => {
                        let temp: SmallVec<[_; 2]> = smallvec![
                            coordinate.add(-1, -1),
                            coordinate.add(-1, 1),
                        ];
    ...
    fn inplace_get_potential_moves(&self, coordinate: &Square, output: &mut MoveContainer) {
        ...
        if color_piece.get_piece() == Piece::Pawn {
            ...
            if (color_piece.get_color() == Color::White && row == 1) || (color_piece.get_color() == Color::Black && row == 6) {
                output.append::<[_; 4]>(&mut pawn_moves
                            .iter()
            ...
            output.append::<[_; 16]>(&mut moves.iter().filter_map(|&coordinate| {
    ...
    pub fn get_allowed_moves(&self, color: Color) -> MoveContainer {
        ...
        output.append::<[_;64]>(&mut maybe_moves
            .iter()
            .filter_map(|&x| {
