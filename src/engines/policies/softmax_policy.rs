...
unsafe impl<E: Evaluator>  Send for SoftmaxPolicy<E> 
where E: Send {}
    
unsafe impl<E: Evaluator>  Sync for SoftmaxPolicy<E> 
where E: Sync {}
...
impl<E: Evaluator> Policy for SoftmaxPolicy<E> {
    fn get_priors(&self, chess_board: &ChessBoard, moves: &MoveContainer) -> SmallVec<[f64; MOVE_CONTAINER_SIZE]> {
        ...
        let mut output: SmallVec<[_; MOVE_CONTAINER_SIZE]> = moves.iter().map(|x|{
            (
                sign * self.evaluator.evaluate(&chess_board.next_state(x))
                / self.temperature
            ).exp()
        }).collect();
