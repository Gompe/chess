...
const EMPTY         : usize =  (BLACK_KING  +  1);
...
const gamephaseInc: [i32; 12] = [0,0,1,1,1,1,2,2,4,4,0,0];
// const mg_table[12][64];
// const eg_table[12][64];

const fn flip(sq: usize) -> usize {
    sq ^ 56
}

fn init_tables() -> ([[i32; 64]; 12], [[i32; 64]; 12])
{

    let mut p = PAWN;
    let mut pc = WHITE_PAWN;
...
trait ChessIndexedEnum {
    fn to_index(&self) -> usize;
}

impl ChessIndexedEnum for Color {
    fn to_index(&self) -> usize {
        match *self {
            Color::White => 0,
            Color::Black => 1
...
impl ChessIndexedEnum for Piece {
    fn to_index(&self) -> usize {
        match *self {
            Piece::Pawn => 0,
            Piece::Knight => 1,
...
impl Evaluator for PestoEvaluator {
    ...
    fn evaluate(&self, chess_board: &crate::chess_server::chess_types::ChessBoard) -> ordered_float::OrderedFloat<f64> {
        ...
        let (mg_table, eg_table) = init_tables();

        for index in 0..64 {
            let square = unsafe { Square::from_index_unchecked(index) };
            let content = chess_board.get_square_content(&square);
            if let Some(content) = content {
                let c = content.get_color().to_index();
                let pc = 2 * content.get_piece().to_index() + c;
                let sq = index as usize;
        ...
        let side_to_move = chess_board.get_turn_color().to_index();

        let mgScore = mg[WHITE] - mg[BLACK];
        let egScore = eg[WHITE] - eg[BLACK];
        let mut mgPhase = game_phase;
        ...
        OrderedFloat( (mgScore * mgPhase + egScore * egPhase) as f64 / 2_400. )
