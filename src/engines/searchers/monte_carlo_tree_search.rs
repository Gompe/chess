    ...
    fn search_internals(
        ...
        let (node_ref, was_visited) = {
            if let Some(node_ref) = self.get_mut_node(chess_board) {
                ...
                let node = Rc::new(RefCell::new(MctsNode::new(allowed_moves, priors, value.0)));
        ...
        match chess_board.get_game_status_from_precomputed(allowed_moves) {
            ChessStatus::Ongoing => {
                ...
                for i in 0..allowed_moves.len() {
                    let frac_action =
                        (1. + total_visits as f64).sqrt() / (1. + node_ref.action_count[i] as f64);
                ...
                drop(node_ref);
                let v = self.search_internals(&chess_board.next_state(&mv), evaluator, depth + 1);
                ...
                node_ref.q_values[best_action.unwrap()] += (((v - node_value) * sign
                    - current_value)
